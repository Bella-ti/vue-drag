...
Vue.directive('dialogdrag', {
  inserted (el, binding) {
    ...
    if (temContainerWidth.indexOf('%') !== -1) {
      ...
      LEFT =
        (parseFloat(temHtmlWidth) -
          (parseFloat(temHtmlWidth) *
            temContainerWidth.substring(0, temContainerWidth.length - 1)) /
            100) /
        2
    } else if (temContainerWidth.indexOf('px') !== -1) {
      // 像素值
      LEFT = (parseFloat(temHtmlWidth) - parseFloat(temContainerWidth)) / 2
    } else {
      // 其他值
      console.log('对话框容器宽度只能为像素或百分比！')
    }
    ...
    if (temMarginTop && temMarginTop.indexOf('px') !== -1) {
      ...
      console.log('请设置对话框容器上边距margin-top并以像素为单位！')
    }
    console.log(LEFT)
    ...
    target.addEventListener('mousedown', function (event) {
      // 获取鼠标距离对话框容器的左上边距
      let leftValue =
        event.clientX - parseFloat(getComputedStyle(container).marginLeft)
      let topValue =
        event.clientY - parseFloat(getComputedStyle(container).marginTop)
